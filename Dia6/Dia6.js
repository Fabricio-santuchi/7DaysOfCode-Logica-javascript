...
function superMercado(){
    ...
    while(conteudo){
        ...
        if(contaItemDentroLista()){
            conteudo = prompt('Se você deseja adicionar uma comida na sua lista de compras Digite (1). Caso queria deletar algum item da sua lista Digite (2). \n(Adicionar (1) / Deletar (2) / sair (3)');   
        }else{
            conteudo = prompt('Se você deseja adicionar uma comida na sua lista de compras? \n(Adicionar (1) / sair (3)');  
        }

        switch(parseInt(conteudo)){
            case 1:
                let alimento = prompt('qual comida você deseja inserir?');
                const categoriaAlimento = prompt('Em qual categoria essa comida se encaixa? \n(frutas, laticinios, congelados, doces)');
