...
function superMercado(){
    let conteudo = true;
    while(conteudo){
        conteudo = prompt('você deseja adicionar uma comida na sua lista de compras? (Sim/Não)');        
        if(conteudo.toLocaleLowerCase() == 'sim'){
            let alimento = prompt('qual comida você deseja inserir?');

            const categoriaAlimento = prompt('Em qual categoria essa comida se encaixa? (frutas, laticinios, congelados, doces)');

            if(categoriaAlimento.toLocaleLowerCase() === 'frutas') listaDeCompras.frutas.push(alimento);
            if(categoriaAlimento.toLocaleLowerCase() === 'laticinios') listaDeCompras.laticinios.push(alimento);
            if(categoriaAlimento.toLocaleLowerCase() === 'congelados') listaDeCompras.congelados.push(alimento);
            if(categoriaAlimento.toLocaleLowerCase() === 'doces') listaDeCompras.doces.push(alimento); 
        }else if(conteudo.toLocaleLowerCase() == 'não' || conteudo.toLocaleLowerCase() == 'nao'){
            conteudo = false;
        }else{
