...
function lerTecnologiasAdicionais() {
  ...
  while (maisTecnologias) {
    const tecnologia = lerEscolha(
      "Digite o nome da tecnologia que você gostaria de aprender:"
    );

    console.log(
      `Excelente escolha! ${tecnologia} é uma tecnologia interessante.`
    );

    const resposta = lerEscolha(
...
function iniciarJogo() {
  ...
  const area = lerEscolha(
    "Você quer seguir para a área de Front-End ou Back-End? (Front-End/Back-End)"
  );
